...
[[ "$(plutil -extract ProgramArguments.0 raw -o - "$PLIST_PATH")" == "$TARGET" ]] || die "LaunchAgent 未直接指向标准二进制：$PLIST_PATH"
[[ "$(plutil -extract ProgramArguments.1 raw -o - "$PLIST_PATH")" == "service" ]] || die "LaunchAgent 缺少 service 命令"
[[ "$(plutil -extract ProgramArguments.2 raw -o - "$PLIST_PATH")" == "launch-core" ]] || die "LaunchAgent 缺少 launch-core 命令"
[[ "$(plutil -extract ProgramArguments.3 raw -o - "$PLIST_PATH")" == "--runtime-root" ]] || die "LaunchAgent 缺少 runtime-root 参数"
[[ "$(plutil -extract ProgramArguments.4 raw -o - "$PLIST_PATH")" == "$APP_SUPPORT_DIR" ]] || die "LaunchAgent runtime-root 不正确"
domain="gui/$(id -u)"
