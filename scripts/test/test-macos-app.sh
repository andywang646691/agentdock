...
CORE_AGENT_PLIST="$APP/Contents/Library/LaunchAgents/com.max.agentdock.core.plist"
TUNNEL_AGENT_PLIST="$APP/Contents/Library/LaunchAgents/com.max.agentdock.tunnel.plist"
MENU_AGENT_PLIST="$APP/Contents/Library/LaunchAgents/com.max.agentdock.menu-login.plist"
test -x "$CORE_HELPER"
...
test "$(plutil -extract Label raw -o - "$CORE_AGENT_PLIST")" = "com.max.agentdock.core"
test "$(plutil -extract BundleProgram raw -o - "$CORE_AGENT_PLIST")" = "Contents/Helpers/agentdock"
test "$(plutil -extract Label raw -o - "$TUNNEL_AGENT_PLIST")" = "com.max.agentdock.tunnel"
...
test "$(plutil -extract Label raw -o - "$MENU_AGENT_PLIST")" = "com.max.agentdock.menu-login"
...
grep -q '^Identifier=com.max.agentdock.login-helper$' <<< "$menu_helper_signature"
grep -q '^Identifier=com.max.agentdock.core$' <<< "$core_signature"
grep -q '^Identifier=com.max.agentdock.cloudflared$' <<< "$cloudflared_signature"
test -f "$DMG"
test -f "$DMG.sha256"
test -f "$ZIP"
test -f "$ZIP.sha256"
plutil -lint "$APP/Contents/Info.plist" >/dev/null
test "$(plutil -extract CFBundleIdentifier raw -o - "$APP/Contents/Info.plist")" = "com.max.agentdock"
...
cmp "$MENU_AGENT_PLIST" "$zip_extract/AgentDock.app/Contents/Library/LaunchAgents/com.max.agentdock.menu-login.plist"
...
cmp "$MENU_AGENT_PLIST" "$MOUNT_POINT/AgentDock.app/Contents/Library/LaunchAgents/com.max.agentdock.menu-login.plist"
